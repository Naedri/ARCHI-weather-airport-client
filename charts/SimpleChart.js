...
export default function SimpleChart(props) {
  ...
  const formatData = (data) => {
    const myObj = {};
    Object.entries(data).forEach(([probeId, v]) =>
      v.forEach((entry) =>
    ...
    return (
      <ResponsiveContainer width="100%" height={350}>
        <LineChart data={data}>
          ...
          <YAxis
            label={{
              value: semanticInfo.yAxis,
              angle: -90,
              position: 'insideLeft',
            }}
          />
          <CartesianGrid stroke="#eee" strokeDasharray="5 5" />
          {Object.keys(dataFromServer ?? {}).map((probeId, index) => (
            <Line type="monotone" key={probeId} dataKey={probeId} connectNulls stroke={colors[index].hex} dot={false} />
          ))}
  ...
  return (
    <div className="container">
      ...
      <div className="bordered">
        <div className="grid grid--gutters">
          <div className="grid__item grid__item--1-5">
            ...
            <select id="iata" value={iata} onChange={({ target }) => setIata(target.value)}>
          ...
          <div className="grid__item grid__item--1-5">
            ...
            <input id="startDate" type="datetime-local" value={startDate} onChange={({ target }) => setStarDate(target.value)} />
          </div>
          <div className="grid__item grid__item--1-5">
            <label htmlFor="endDate">End date : </label>
            <input id="endDate" type="datetime-local" value={endDate} onChange={({ target }) => setEndDate(target.value)} />
          </div>
          <div className="grid__item grid__item--1-5">
            <label htmlFor="dataType">Data type : </label>
            <select id="dataType" value={dataType} onChange={({ target }) => setDataType(target.value)}>
              <option value="" disabled>
                --Please choose a type of mesure--
              </option>
          ...
          <div className="grid__item grid__item--1-5">
            ...
            <button id="refresh" onClick={() => queryClient.invalidateQueries('data')}>
              🔄
            </button>
          </div>
        </div>
      ...
      <input id="meanDate" type="date" value={meanDate} onChange={({ target }) => setMeanDate(target.value)} />
      {average && (
        <>
          <ul>
            {Object.entries(average).map(([k, v], i) => {
              let type = getSemanticInfo(k);
              return <li key={i}>{`${type.name}: ${Number(v.toFixed(1))} ${type.unit}`}</li>;
            })}
          </ul>
        </>
      )}
      <div style={{ padding: '30px' }}>{chartMemo}</div>
    </div>
  );
}
